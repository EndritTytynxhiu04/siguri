...
def upload_file():
    ...
    try:
        ...
        client_public_pem = serialize_public_key(
            client_public_key
        )
        ...
        send_msg(
            client_socket,
            f"UPLOAD|{filename}".encode("utf-8")
        )
        ...
        send_msg(client_socket, signature)
...
def download_file():
    ...
    try:
        ...
        send_msg(
            client_socket,
            f"DOWNLOAD|{filename}".encode("utf-8")
        )
        ...
        if not status or status.startswith(b"ERROR"):
            print(
                f"[-] Server error: "
                f"{status.decode('utf-8') if status else 'Unknown Error'}"
            )
            return
        ...
        print(
            f"[+] File '{filename}' successfully "
            f"downloaded and saved to {save_path}"
        )
